...
class DEX:
    def __init__(self, name, network='testnet'):
        ...
        self.router_address, self.factory_address, self.abi_paths = get_dex_info(self.name, self.network)
        ...
        with open("config/{}".format(self.abi_paths['factory']), encoding='utf-8', errors='ignore') as json_data:
            ...
            self.factory_contract = self.rpc.eth.contract(address=self.factory_address, abi=self.factory_abi)
        ...
        with open("config/{}".format(self.abi_paths['router']), encoding='utf-8', errors='ignore') as json_data:
            ...
            self.router_contract = self.rpc.eth.contract(address=self.router_address, abi=self.router_abi)

    def swap(self, account, token_from, token_to, amount, slippage):
        # approve token
        if token_from != 'eth':
            value = int(Decimal(amount * 10 ** 6))
            approved_amount = self.check_approval(account, token_from, self.router_address)
    ...
    def approve_token(self, account, token, amount):
        if len(token) < 10:
            ...
            token_contract = self.rpc.eth.contract(address=self.rpc.to_checksum_address(token),
                                                   abi=token_abi)
        func = token_contract.functions.approve(self.router_address, int(Decimal(amount * 10 ** 6)))
    ...
    def check_approval(self, account, token, spender_address):
        if len(token) < 10:
            ...
            token_contract = self.rpc.eth.contract(address=self.rpc.to_checksum_address(token),
                                                   abi=token_abi)
        approved_amount = token_contract.functions.allowance(account.address, spender_address).call()
